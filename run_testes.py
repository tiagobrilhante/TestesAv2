...
def flush_input():
    """Limpa o buffer de entrada para evitar capturas indesejadas."""
    try:
        ...
        try:
            ...
            # Método alternativo: ler e descartar entrada
            try:
                while select.select([sys.stdin], [], [], 0)[0]:
                    sys.stdin.read(1)
            except (ImportError, IOError, AttributeError):
                pass  # Se nenhum método funcionar, continua sem limpar
...
def run_specific_test(test_name):
    ...
    pytest.main(["-v", "-s", f"-k {test_name}"])

    print_step("Teste concluído", "Verificando resultados")

    # Limpar o buffer de entrada antes de solicitar o input
    flush_input()
...
def show_manual():
    ...
    tests_info = [
        ("1. Testes de Atualização de Dados",
         "Verifica se é possível atualizar registros no banco de dados corretamente."),
        ("2. Testes de JOIN",
         "Testa a operação de junção entre tabelas para combinar dados relacionados."),
        ("3. Testes de View",
         "Verifica a criação e execução de views (visões) no banco de dados."),
        ("4. Testes de Trigger",
         "Testa a execução automática de triggers quando ocorrem eventos específicos."),
        ("5. Testes de Integridade Referencial",
         "Verifica se as restrições de chave estrangeira estão funcionando corretamente."),
        ("6. Testes de Performance de Índice",
         "Avalia o impacto dos índices na performance de consultas."),
        ("7. Testes de Transação e Rollback",
         "Testa a capacidade de reverter operações em caso de erro."),
        ("8. Testes de Consultas Parametrizadas",
         "Verifica a segurança contra SQL Injection usando consultas parametrizadas."),
        ("9. Testes de Consultas com LIKE",
         "Testa consultas usando o operador LIKE para busca de padrões."),
        ("10. Testes de Deleção em Cascata",
         "Verifica se a deleção em cascata funciona corretamente para manter a integridade."),
        ("11. Testes de Performance de Inserção em Lote",
...
def main_menu():
    ...
    while True:
        ...
        print(f"{Fore.GREEN}1.{Style.RESET_ALL} Testes de Atualização de Dados")
        print(f"{Fore.GREEN}2.{Style.RESET_ALL} Testes de JOIN")
        print(f"{Fore.GREEN}3.{Style.RESET_ALL} Testes de View")
        print(f"{Fore.GREEN}4.{Style.RESET_ALL} Testes de Trigger")
        print(f"{Fore.GREEN}5.{Style.RESET_ALL} Testes de Integridade Referencial")
        print(f"{Fore.GREEN}6.{Style.RESET_ALL} Testes de Performance de Índice")
        print(f"{Fore.GREEN}7.{Style.RESET_ALL} Testes de Transação e Rollback")
        print(f"{Fore.GREEN}8.{Style.RESET_ALL} Testes de Consultas Parametrizadas")
        print(f"{Fore.GREEN}9.{Style.RESET_ALL} Testes de Consultas com LIKE")
        print(f"{Fore.GREEN}10.{Style.RESET_ALL} Testes de Deleção em Cascata")
        print(f"{Fore.GREEN}11.{Style.RESET_ALL} Testes de Performance de Inserção em Lote")
        ...
        if opcao == '1':
            run_specific_test("test_update_user")
        elif opcao == '2':
            run_specific_test("test_join_users_orders")
        elif opcao == '3':
            run_specific_test("test_view_execution")
        elif opcao == '4':
            run_specific_test("test_trigger_execution")
        elif opcao == '5':
            run_specific_test("test_foreign_key_constraint")
        elif opcao == '6':
